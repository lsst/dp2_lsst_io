...
#########################################
102.1. Catalog access with PyVO using TAP
#########################################
...
**Learning objective:** Query Rubin DP2 catalogs using the Table Access Protocol (TAP) with PyVO from a Python environment.

**LSST data products:** DP2 Visit catalogs accessed via the Rubin Science Platform TAP service.
...
This tutorial demonstrates how to access Rubin DP2 catalog data programmatically using the API Aspect of the Rubin Science Platform.
The tutorial uses the Python Virtual Observatory (PyVO) package to authenticate with the Rubin Table Access Protocol (TAP) service and execute an asynchronous Astronomical Data Query Language (ADQL) query.
...
Note that in certain environments like Google Colab you may have to re-install these packages with a ``pip install pyvo`` for each session.
...
Define an ADQL query that retrieves the ``visit`` table, as in `DP2 Notebook tutorial 201.10 <https://dp2.lsst.io/tutorials/notebook/201/notebook-201-10.html>`_.
...
In this case, the length should be 114.
...
.. figure:: images/api-102-1-1.png
   ...
   Figure 1: Preview of the DP2 ``visit`` table query results returned by the TAP service.
