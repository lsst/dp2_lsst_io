...
See examples of queries with spatial constraints in API Tutorial 101.1 (linked above) or step 3.1 below.**
...
The default plot shows the fields contained in Data Preview 2 on a rotating globe.
...
Return to the Table Access Protocol (TAP) Query window to execute a new ADQL query.
Copy the ADQL query below, add it to the window in the "ADQL Text" box (see Figure 1) and click "Run Query".
A temporary "Load New Table" window will appear and a new table will appear in the main TOPCAT window (see Figure 4) when the query is complete.
...
In the "Histogram Plot" window the default histogram will be plotted from the first appropriate value in the drop-down menu (in this case the "ra").
